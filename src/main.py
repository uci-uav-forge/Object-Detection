...
def main():
    ...
    #unpack both images for shape and letter detection.
    ...
    #fill in is to get rid of letter inside detected shape, so it won't mess with contour detection.
    shape_img = cv2.cvtColor(color_img, cv2.COLOR_BGR2GRAY)
    shape_img = fill_in(shape_img)

    
    #shape_detection / center coordinate stuff
    ...
    #uses bounding box to get only the detected shape in order to do letter_recognition
